...
export const canEditAccessRequestGuard: CanActivateFn = (route) => {
  ...
  return accessRequestResolver.resolve(route).pipe(
    map((accessRequest: AccessRequest) => {
      if (accessRequest?.cruved?.U) {
        return true;
      }
      return redirectToAccessRequest();
    }),
    catchError(() => of(redirectToAccessRequest()))
  );
};
