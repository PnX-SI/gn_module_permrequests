...
from gn_module_access_request.models import AccessRequest
...
def test_list_access_requests_returns_latest(client, users, taxon_ids):
    author = users["admin_user"]
    first = create_access_request(author, description="List request 1", taxa_ids=taxon_ids[:2])
    ...
    with logged_user(client, author):
        response = client.get(
            "/access_request/",
            query_string={"orderby": "id_access_request", "sort": "desc", "per_page": 5},
        )

    assert response.status_code == 200
    payload = response.get_json()
    ids = [item["id_access_request"] for item in payload["items"]]
    assert second.id_access_request == payload["items"][0]["id_access_request"]
    assert first.id_access_request in ids


def test_list_access_requests_respects_scope(client, users, taxon_ids):
    other_request = create_access_request(users["admin_user"], description="Hidden", taxa_ids=taxon_ids[:2])
    self_request = create_access_request(users["self_user"], description="Visible", taxa_ids=taxon_ids[:2])

    with logged_user(client, users["self_user"]):
        response = client.get(
            "/access_request/",
            query_string={"orderby": "id_access_request", "sort": "desc", "per_page": 10},
        )

    assert response.status_code == 200
    ids = [item["id_access_request"] for item in response.get_json()["items"]]
    assert self_request.id_access_request in ids
    assert other_request.id_access_request not in ids


def test_get_access_request_returns_payload(client, users, taxon_ids, area_ids):
    ...
    with logged_user(client, users["admin_user"]):
        response = client.get(f"/access_request/{created.id_access_request}")

    assert response.status_code == 200
    data = response.get_json()
    assert data["id_access_request"] == created.id_access_request
    assert data["description"] == "Fetch"
    assert len(data["taxa"]) == len(taxon_ids[:2])
    assert [area["id_area"] for area in data["areas"]] == [area_ids[0]]


def test_get_access_request_not_found(client, users):
    with logged_user(client, users["admin_user"]):
        response = client.get("/access_request/-1")

    assert response.status_code == 404


def test_create_access_request_success(client, users, taxon_ids, area_ids):
    initialization = date.today()
    ...
    with logged_user(client, users["admin_user"]):
        response = client.post("/access_request/", json=payload)

    assert response.status_code == 201
    data = response.get_json()
    assert data["description"] == payload["description"]
    assert data["id_author"] == users["admin_user"].id_role
    created = db.session.get(AccessRequest, data["id_access_request"])
...
def test_create_access_request_rejects_invalid_taxa(client, users):
    initialization = date.today()
    payload = {
...
def test_create_access_request_rejects_invalid_areas(client, users, taxon_ids):
    initialization = date.today()
    payload = {
...
def test_update_access_request_updates_fields(client, users, taxon_ids, area_ids):
    created = create_access_request(
        users["admin_user"],
    ...
    with logged_user(client, users["admin_user"]):
        response = client.patch(f"/access_request/{created.id_access_request}", json=update_payload)
...
def test_delete_access_request_removes_entry(client, users, taxon_ids):
    created = create_access_request(users["admin_user"], description="To delete", taxa_ids=taxon_ids[:2])

    with logged_user(client, users["admin_user"]):
        response = client.delete(f"/access_request/{created.id_access_request}")

    assert response.status_code == 204
    assert db.session.get(AccessRequest, created.id_access_request) is None


def test_update_validated_sets_validator(client, users, taxon_ids):
    created = create_access_request(users["admin_user"], description="Needs validation", taxa_ids=taxon_ids[:2])
