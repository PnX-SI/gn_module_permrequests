...
ACCESS_REQUEST_DESCRIPTION_PREFIX = "ar_sample__access_request_"
...
def _create_access_request_permissions(
    conn,
    authors,
    validator_ids,
    group_role_id,
    ...
    if not area_ids:
        raise RuntimeError("Unable to create sample access requests without geographic areas.")
    if not taxon_ids:
        raise RuntimeError("Unable to create sample access requests without taxa.")

    random.seed(42)
    base_date = date.today()
    template_count = len(ACCESS_REQUEST_TEMPLATES)
    for index in range(ACCESS_REQUEST_SAMPLE_SIZE):
        template = ACCESS_REQUEST_TEMPLATES[index % template_count]
...
def upgrade():
    ...
    if not area_ids:
        raise RuntimeError("Unable to fetch reference areas for sample access requests.")
    if not taxon_ids:
        raise RuntimeError("Unable to fetch reference taxa for sample access requests.")
    ...
    _create_access_request_permissions(
        conn,
        requester_ids,
        validator_ids,
        group_role_id,
...
def _cleanup_access_requests(conn):
    rows = conn.execute(
        sa.text(
            f"""
            SELECT {PRIMARY_KEY} AS id_access_request, id_permission
            FROM {SCHEMA_NAME}.{TABLE_NAME}
            WHERE description LIKE :prefix
            """
        ),
        {"prefix": f"{ACCESS_REQUEST_DESCRIPTION_PREFIX}%"},
    ).mappings().all()
    ...
    for row in rows:
        conn.execute(
            ...
            {"id_access_request": row["id_access_request"]},
        )
